...
function updateSubjectDisplay() {
    ...
    const containers =
        document.querySelectorAll("[data-subject-list], #subjectList");
...
function setupGoogleCalendarSync() {
    ...
    button.onclick = function () {
        ...
        const studySessions = plan.filter(
            session => session.subject !== "Break"
        );

        if (!studySessions.length) {
            alert("No study sessions found.");
            return;
        }

        // Start from today at 6:00 PM
        let start = new Date();
        ...
        const end = new Date(
            start.getTime() +
            (Number(session.minutes) || 25) * 60000
        );
        ...
        const title =
            "FOCLYRA Study: " + session.subject;

        const details =
            "Study session planned by FOCLYRA.";
